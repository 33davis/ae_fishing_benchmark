#!/usr/bin/bash -l
#PBS -N kddm2_MARESPR2_syn_solofq
...
/scratch/davisee/synthetic_files/fq_input/solo_fq2_combo
...
align="MALT"
new_directory="/scratch/davisee/synthetic_files/fq_input/solo_fq2_combo/$d-$database"
mkdir "$new_directory"

############## Komplexity #####################
for sample in *.fq; do kz --filter --threshold 0.55 < "$sample" > "$sample.kx"; done

# Move komplexity results to new directory
mv /scratch/davisee/synthetic_files/fq_input/solo_fq2_combo/*.kx "$new_directory/"

################## Dedupe ######################
cd -- "$new_directory"
for sample in *.kx; do dedupe.sh in="$sample" out="$sample-dd.gz" outd="$sample-duplicates.fa" ac=f -Xmx80g tossbrokenreads; done
